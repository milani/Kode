...
class CKEditor {
    ...
    public function replace($id, $config = array(), $events = array()){
        ...
        $js = $this->returnGlobalEvents();
        if( ! empty($_config) ){
            $js .= "CKEDITOR.replace('" . $id . "', " . $this->jsEncode(
            $_config) . ");";
        }else{
    ...
    private function init(){
        ...
        $out .= "<script type=\"text/javascript\" src=\"" . $ckeditorPath .
         'ckeditor.js' . $args . "\"></script>\n";
        $extraCode = "";
    ...
    private function ckeditorPath(){
        ...
        $documentRoot = substr($realPath, 0, 
        strlen($realPath) - strlen($selfPath));
