 ...
 */
class App_View_Helper_AttachmentUtility extends Zend_View_Helper_Abstract {
    ...
    protected function _init(array $config){
        ...
        if( is_string($tmp) ){
            ...
            if( ! isset($tmp['defaultParams']) ||
             ! is_array($tmp['defaultParams']) )
                $this->_linkRouter['defaultParams'] = array();
        }else{
            $this->_linkRouter['name'] = Zend_Controller_Front::getInstance()->getRouter()->getCurrentRouteName();
            $this->_linkRouter['defaultParams'] = array();
    ...
    public function link($link, array $item = array(), $echo = TRUE){
        ...
        if( $action['parameter'] ){
            if( ! isset($item['id']) ){
                ...
                'This action requires a parameter. Please pass the correct array to this method.');
            }
            if( ! isset($action['parameter_name'])) $action['parameter_name'] = 'id';
            $urlParams[$action['parameter_name']] = $item['id'];
        }
        ...
        if( ! isset($action['icon']) || (isset($action['icon']) && $action['icon']) ){
            ...
            $link = '<li>' . PHP_EOL . '<a class="' . $action['link_class'] .
    ...
    public function displayItem($item, $index = NULL, $echo = TRUE){
        ...
        if( $viewLink && $this->can('view') ){
            $urlParams = array(
                'controller' => $this->_controllerName, 'action' => 'view', 
            'id' => $itemId
            );
            $result = sprintf('<a href="%1$s" title="%2$s">%3$s</a>', 
            $this->_assembleUrl($urlParams), 
            sprintf('View details for %s', $result), $result);
        }
        return $this->_return($result, $echo);
    }
    ...
    protected function _return($string, $echo){
        if( $echo ){
            echo ($this->_enableTranslation) ? Zend_Registry::get(
            'Zend_Translate')->translate($string) : $string;
        }else{
            return ($this->_enableTranslation) ? Zend_Registry::get(
            'Zend_Translate')->translate($string) : $string;
        }
    }
