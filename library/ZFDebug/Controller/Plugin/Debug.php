...
class ZFDebug_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract {
    ...
    'plugins' => array(
        'Variables' => null, 'Time' => null, 'Memory' => null
    ), 'z-index' => 255, 'image_path' => null
    );
    ...
    public static $standardPlugins = array(
        'Cache', 'Html', 'Database', 'Exception', 'File', 'Memory', 
    'Registry', 'Time', 'Variables'
    );
    ...
    public function __construct($options = null){
        ...
        if( isset($options) ){
            ...
            if( ! is_array($options) ){
                ...
                'Debug parameters must be in an array or a Zend_Config object');
            }
    ...
    ZFDebug_Controller_Plugin_Debug_Plugin_Interface $plugin){
    ...
    public function dispatchLoopShutdown(){
        ...
        foreach( $this->_plugins as $plugin ){
            ...
            /* @var $plugin ZFDebug_Controller_Plugin_Debug_Plugin_Interface */
            $html .= '<span class="ZFDebug_span clickable" onclick="ZFDebugPanel(\'ZFDebug_' .
             $plugin->getIdentifier() . '\');">';
            $html .= '<img src="' . $this->_icon($plugin->getIdentifier()) .
             '" style="vertical-align:middle" alt="' . $plugin->getIdentifier() .
    ...
    protected function _loadPlugins(){
        ...
        foreach( $this->_options['plugins'] as $plugin => $options ){
            ...
            ZFDebug_Controller_Plugin_Debug::$standardPlugins) ){
                // standard plugin
                $pluginClass = 'ZFDebug_Controller_Plugin_Debug_Plugin_' .
                 $plugin;
            }else{
                // we use a custom plugin
                if( ! preg_match('~^[\w]+$~D', $plugin) ){
                    throw new Zend_Exception(
                    "ZFDebug: Invalid plugin name [$plugin]");
                }
                $pluginClass = $plugin;
            }
            require_once str_replace('_', DIRECTORY_SEPARATOR, $pluginClass) .
             '.php';
    ...
    protected function _output($html){
        ...
        $response->setBody(
        str_ireplace('</body>', 
        '<div id="ZFDebug_debug">' . $html . '</div></body>', 
        $response->getBody()));
    }
}