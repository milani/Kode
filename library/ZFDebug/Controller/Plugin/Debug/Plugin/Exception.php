 ...
 * @license    http://code.google.com/p/zfdebug/wiki/License     New BSD License
 */
class ZFDebug_Controller_Plugin_Debug_Plugin_Exception implements 
ZFDebug_Controller_Plugin_Debug_Plugin_Interface {
    ...
    public function getTab(){
        ...
        $text = $exception . ($exception == '' || $error == '' ? '' : ' - ') .
         $error;
        return $text;
    }
    ...
    public function getPanel(){
        ...
        foreach( $response->getException() as $e ){
            $html .= '<h4>' . get_class($e) . ': ' . $e->getMessage() .
             '</h4><p>thrown in ' . $e->getFile() . ' on line ' . $e->getLine() .
             '</p>';
        ...
        if( $errorCount ){
            ...
            foreach( self::$errors as $error ){
                $html .= '<li>' . sprintf("%s: %s in %s on line %d", 
    ...
    public static function errorHandler($level, $message, $file, $line){
        ...
        self::$errors[] = array(
            'type' => $type, 'message' => $message, 'file' => $file, 
        'line' => $line
        );
        if( ini_get('log_errors') )
            error_log(
            sprintf("%s: %s in %s on line %d", $type, $message, $file, $line));
        return true;
    }
}