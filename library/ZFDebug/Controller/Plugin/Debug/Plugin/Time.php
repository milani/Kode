 ...
 * @license    http://code.google.com/p/zfdebug/wiki/License     New BSD License
 */
class ZFDebug_Controller_Plugin_Debug_Plugin_Time extends Zend_Controller_Plugin_Abstract implements 
ZFDebug_Controller_Plugin_Debug_Plugin_Interface {
    ...
    public function getPanel(){
        ...
        $html .= 'Controller: ' . round(
        ($this->_timer['postDispatch'] - $this->_timer['preDispatch']), 2) .
         ' ms<br />';
        ...
        foreach( $timerNamespace->data as $module => $controller ){
            ...
            foreach( $controller as $con => $action ){
                ...
                foreach( $action as $key => $data ){
                    ...
                    $html .= '        ' . $key . '<br />';
                    $html .= '<div class="pre">';
                    $html .= '            Avg: ' . $this->_calcAvg($data) .
                     ' ms / ' . count($data) . ' requests<br />';
                    $html .= '            Min: ' . round(min($data), 2) .
                     ' ms<br />';
                    $html .= '            Max: ' . round(max($data), 2) .
                     ' ms<br />';
                    $html .= '</div>';
                }
                $html .= '</div>';
    ...
    public function mark($name){
        ...
        if( isset($this->_timer['user'][$name]) )
            $this->_timer['user'][$name] = (microtime(true) -
             $_SERVER['REQUEST_TIME']) * 1000 - $this->_timer['user'][$name];
        else
            $this->_timer['user'][$name] = (microtime(true) -
    ...
    public function preDispatch(Zend_Controller_Request_Abstract $request){
        ...
        $reset = Zend_Controller_Front::getInstance()->getRequest()->getParam(
        'ZFDEBUG_RESET');
        if( isset($reset) ){
            $timerNamespace = new Zend_Session_Namespace('ZFDebug_Time', false);
            $timerNamespace->unsetAll();
        }
        $this->_timer['preDispatch'] = (microtime(true) -
         $_SERVER['REQUEST_TIME']) * 1000;
    }
    ...
    public function postDispatch(Zend_Controller_Request_Abstract $request){
        ...
        $this->_timer['postDispatch'] = (microtime(true) -
         $_SERVER['REQUEST_TIME']) * 1000;
    }
