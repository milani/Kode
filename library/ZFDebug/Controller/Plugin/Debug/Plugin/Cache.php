 ...
 * @license    http://code.google.com/p/zfdebug/wiki/License     New BSD License
 */
class ZFDebug_Controller_Plugin_Debug_Plugin_Cache implements 
ZFDebug_Controller_Plugin_Debug_Plugin_Interface {
    ...
    public function __construct(array $options = array()){
        ...
        if( ! isset($options['backend']) ){
            ...
            "ZFDebug: Cache plugin needs 'backend' parameter");
        }
        is_array($options['backend']) || $options['backend'] = array(
            $options['backend']
        );
    ...
    public function getPanel(){
        ...
        if( function_exists('apc_sma_info') && ini_get('apc.enabled') ){
            ...
            $panel .= round($mem_avail / 1024 / 1024, 1) . 'M available, ' .
             round($mem_used / 1024 / 1024, 1) . 'M used<br />' .
             $cache['num_entries'] . ' Files cached (' .
             round($cache['mem_size'] / 1024 / 1024, 1) . 'M)<br />' .
             $cache['num_hits'] . ' Hits (' . round(
            $cache['num_hits'] * 100 /
        ...
        foreach( $this->_cacheBackends as $name => $backend ){
            ...
            $panel .= '<h4>Cache ' . $name . ' (' .
             get_class($backend) . ')</h4>';
            $panel .= count($ids) . ' Entr' . (count($ids) > 1 ? 'ies' : 'y') .
             '<br />' . 'Filling Percentage: ' . $backend->getFillingPercentage() .
             '%<br />';
            $cacheSize = 0;
            foreach( $ids as $id ){
