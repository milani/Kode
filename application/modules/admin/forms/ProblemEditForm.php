 ...
 */
class ProblemEditForm extends ProblemForm {
    ...
    public function populate($values){
        ...
        $rows = $problemModel->findRelatedClasses($id);
        if(count($rows) > 1){
            $desc = array();
            foreach($rows as $row){
                $desc[] = $row['class_name'];
            ...
            $forkElem->setChecked(false);
        }else{
            $forkElem->setChecked(true);
            $forkElem->setAttrib('disabled', 'disabled');
            $forkElem->setDescription('This problem is only available in this class.');
        }
